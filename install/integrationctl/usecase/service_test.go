...
func TestUpdateAllDryRunAggregatesManagedIntegrations(t *testing.T) {
	...
	writeFile(t, filepath.Join(root, "cursor-demo", "src", "plugin.yaml"), "api_version: v1\nname: cursor-demo\nversion: 0.2.0\ndescription: test\ntargets:\n  - cursor\n")
	writeFile(t, filepath.Join(root, "opencode-demo", "src", "plugin.yaml"), "api_version: v1\nname: opencode-demo\nversion: 0.2.0\ndescription: test\ntargets:\n  - opencode\n")
	fs := fsadapter.OS{}
	store := jsonstate.Store{FS: fs, Path: filepath.Join(root, "state.json")}
...
func TestAddNonDryRunInstallsCursorAndPersistsState(t *testing.T) {
	...
	writeFile(t, filepath.Join(sourceRoot, "src", "mcp", "servers.yaml"), "api_version: v1\nservers:\n  release-checks:\n    type: stdio\n    stdio:\n      command: node\n      args:\n        - ${package.root}/bin/release-checks.mjs\n")
	evidencePath := filepath.Join(root, "evidence.json")
...
func TestAddNonDryRunInstallsMultipleTargetsAndPersistsUnifiedState(t *testing.T) {
	...
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "package.yaml"), "plugins:\n  - '@acme/opencode-demo-plugin'\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "plugins", "example.js"), "export const ExamplePlugin = async () => ({})\n")
	evidencePath := filepath.Join(root, "evidence.json")
...
func TestUpdateNonDryRunRefreshesMultipleTargetsAndPersistsUnifiedState(t *testing.T) {
	...
	writeFile(t, filepath.Join(sourceRoot, "src", "mcp", "servers.yaml"), "api_version: v1\nservers:\n  release-checks:\n    type: stdio\n    stdio:\n      command: node\n      args:\n        - ${package.root}/bin/v1.mjs\n    targets:\n      - cursor\n      - opencode\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "package.yaml"), "plugins:\n  - '@acme/opencode-demo-plugin'\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "plugins", "example.js"), "v1\n")
	evidencePath := filepath.Join(root, "evidence.json")
	writeFile(t, evidencePath, `{"schema_version":1,"entries":[{"key":"target.cursor.native_surface","claim":"x","evidence_class":"confirmed_vendor_fact","urls":["https://example.com"]},{"key":"target.opencode.native_surface","claim":"x","evidence_class":"confirmed_vendor_fact","urls":["https://example.com"]}]}`)
	fs := fsadapter.OS{}
	projectRoot := filepath.Join(root, "workspace")
	...
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "plugins", "example.js"), "v2\n")
...
func TestUpdateDryRunCleansMaterializedResolvedSource(t *testing.T) {
	...
	writeFile(t, filepath.Join(localSourceRoot, "src", "plugin.yaml"), "api_version: v1\nname: cursor-demo\nversion: 0.1.0\ndescription: test\ntargets:\n  - cursor\n")
	writeFile(t, filepath.Join(localSourceRoot, "src", "mcp", "servers.yaml"), "api_version: v1\nservers:\n  release-checks:\n    type: stdio\n    stdio:\n      command: node\n      args:\n        - ${package.root}/bin/v1.mjs\n")
	evidencePath := filepath.Join(root, "evidence.json")
	writeFile(t, evidencePath, `{"schema_version":1,"entries":[{"key":"target.cursor.native_surface","claim":"x","evidence_class":"confirmed_vendor_fact","urls":["https://example.com"]}]}`)
	fs := fsadapter.OS{}
	projectRoot := filepath.Join(root, "workspace")
	...
	writeFile(t, filepath.Join(materializedPlugin, "src", "plugin.yaml"), "api_version: v1\nname: cursor-demo\nversion: 0.2.0\ndescription: test\ntargets:\n  - cursor\n")
	writeFile(t, filepath.Join(materializedPlugin, "src", "mcp", "servers.yaml"), "api_version: v1\nservers:\n  release-checks:\n    type: stdio\n    stdio:\n      command: node\n      args:\n        - ${package.root}/bin/v2.mjs\n")
...
func TestAddNonDryRunInstallsOpenCodeAndPersistsState(t *testing.T) {
	...
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "package.yaml"), "plugins:\n  - '@acme/opencode-demo-plugin'\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "plugins", "example.js"), "export const ExamplePlugin = async () => ({})\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "package.json"), "{\n  \"name\": \"demo\",\n  \"private\": true\n}\n")
	evidencePath := filepath.Join(root, "evidence.json")
...
func TestRepairNonDryRunRestoresOpenCodeManagedEntries(t *testing.T) {
	...
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "package.yaml"), "plugins:\n  - '@acme/opencode-demo-plugin'\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "plugins", "example.js"), "v1\n")
	writeFile(t, filepath.Join(sourceRoot, "src", "targets", "opencode", "permission.json"), "{\"bash\":\"ask\"}\n")
	evidencePath := filepath.Join(root, "evidence.json")
	writeFile(t, evidencePath, `{"schema_version":1,"entries":[{"key":"target.opencode.native_surface","claim":"x","evidence_class":"confirmed_vendor_fact","urls":["https://example.com"]}]}`)
	fs := fsadapter.OS{}
	projectRoot := filepath.Join(root, "workspace")
