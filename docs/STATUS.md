...
| Cursor packaged lane | done | Packaged Cursor lane now uses `.cursor-plugin/plugin.json` plus portable `plugin/skills/**` and optional shared `.mcp.json` as the primary public path. Legacy `src/` repos remain supported. The older repo-local `.cursor` subset remains available only as the secondary `cursor-workspace` target. |
