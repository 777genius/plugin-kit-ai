...
| Codex GA path | done | Runtime, scaffold, validate, integration coverage, and repository-owned opt-in real `codex exec` smoke plus real `codex mcp get --json` preflight exist. The supported invocation semantics are now frozen in the release audit, external runtime-health failures are explicitly outside the plugin-kit-ai stable promise, and current live evidence also records that Codex CLI `v0.117.0` does not reliably honor project-local `.codex/config.toml` for `exec` or `mcp get`. |
...
| Cursor workspace lane | done | Documented Cursor MCP + rules subset now has render/import/validate coverage, optional shared `AGENTS.md` support, and an explicit deferred-scope boundary for `CLAUDE.md`, global MCP, nested rules, and JSONC. |
