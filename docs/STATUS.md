...
| Codex GA path | done | Runtime, scaffold, validate, integration coverage, repository-owned opt-in real `codex exec` smoke, real `codex mcp get --json` preflight, and rendered `codex-package` `.mcp.json` sidecar preflight exist. The supported invocation semantics are now frozen in the release audit, external runtime-health failures are explicitly outside the plugin-kit-ai stable promise, and current live evidence also records that Codex CLI `v0.117.0` does not reliably honor project-local `.codex/config.toml` for `exec` or `mcp get`. |
