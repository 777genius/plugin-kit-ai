...
- OpenCode workspace-config lane through `plugin-kit-ai render|import|validate`, covering official-style `opencode.json`, package refs, inline `mcp`, mirrored `.opencode/skills/`, `config.extra.json`, passthrough config surfaces like `agent` and `tools`, and explicit warnings for unsupported local plugin code plus unsupported workspace directories such as `agents`, `commands`, `themes`, and `modes`
