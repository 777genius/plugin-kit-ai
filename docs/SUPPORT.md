...
- live Codex CLI evidence currently splits into two buckets:
  ...
  - evidence-only project-config probes: on the current live Codex CLI build (`v0.117.0` in repo evidence), `codex exec`, `codex mcp get`, and `codex mcp list` do not reliably honor project-local `.codex/config.toml`; the corresponding opt-in live tests therefore record `skip` with captured output instead of claiming support that the vendor CLI did not prove
