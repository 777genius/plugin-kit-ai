...
- `mcp/servers.json` already fits our portable MCP pipeline
...
- reuse portable MCP authored source `mcp/servers.json`
...
- `mcp/servers.json` should remain user-authored, not force-generated by scaffold when empty
...
- optional `mcp/servers.json` -> `.cursor/mcp.json`
...
- native MCP goes back into `mcp/servers.json`
...
- `mcp/servers.json` parses as JSON object and renders successfully to `.cursor/mcp.json`
...
- rendered `.cursor/mcp.json` only if `mcp/servers.json` exists
