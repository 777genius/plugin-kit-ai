...
| `notion` | Pass | Codex; Cursor local load | Authenticated read-only search through raw ChatGPT MCP only | Passed for ChatGPT raw MCP; Cursor package consent pending | ChatGPT; Cursor; direct harness; Codex discovery |
...
| Interactive hero runtime matrix | 5 local packages | Client-specific test loading in Codex, Cursor, and Kiro | 12/12 no-auth checks passed across 3 clients; Notion is pending | 0/3 package OAuth; consent in progress |
