...
- Gemini now has a scoped `public-beta` Go runtime lane for `SessionStart`, `SessionEnd`, `BeforeModel`, `AfterModel`, `BeforeToolSelection`, `BeforeAgent`, `AfterAgent`, `BeforeTool`, and `AfterTool`, and continues hardening inside the package-standard surface without widening the stable runtime promise
