...
- OpenCode: workspace-config-only lane through `render|import|validate`, package refs, inline MCP, validated portable skills, first-class workspace commands/agents/themes, beta standalone tools, stable local plugin code plus stable shared package metadata for tools and plugins, JSON/JSONC plus explicit user-scope import compatibility, and beta `custom_tools` spanning standalone tools and plugin code
