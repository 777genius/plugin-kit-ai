...
- `init --platform cursor`: packaged Cursor plugin starter with `src/plugin.yaml`, root boundary docs `CLAUDE.md` and `AGENTS.md`, optional portable `src/mcp/servers.yaml` and `src/skills/**` when `--extras` is used, and generated `.cursor-plugin/plugin.json`; use `cursor-workspace` only when you intentionally want the secondary repo-local `.cursor/` config lane
- `generate`: generate native Claude artifacts, Codex package/runtime lane artifacts, Gemini CLI extension packaging artifacts, OpenCode workspace config artifacts, packaged Cursor plugin artifacts, and the secondary Cursor workspace config artifacts from canonical `src/plugin.yaml`, optional `src/mcp/servers.yaml`, optional `src/skills/**`, and `src/targets/<platform>/...`; when `src/publish/codex/marketplace.yaml` is authored for `codex-package`, `generate` also produces repo-level `.agents/plugins/marketplace.json`, and when `src/publish/claude/marketplace.yaml` is authored for `claude`, `generate` also produces `.claude-plugin/marketplace.json`
