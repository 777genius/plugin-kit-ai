...
- `publication`: focused publication-layer view for package-capable targets and authored `publish/...` channels; use it when you want only package/channel publication data without the broader inspect surface; `publication --format json` emits the versioned `plugin-kit-ai/publication-report` contract, and `publication doctor --format json` emits `plugin-kit-ai/publication-doctor-report` for readiness gating
