...
- [claude-basic-prod](./claude-basic-prod): Claude plugin repo with `src/plugin.yaml`, generated native artifacts, and deterministic local smoke path
- [codex-basic-prod](./codex-basic-prod): Codex runtime lane repo with `src/plugin.yaml`, generated `.codex/config.toml`, deterministic local notify smoke path, and repo-local MCP passthrough example
- [codex-package-prod](./codex-package-prod): official Codex package lane with `src/plugin.yaml`, generated `.codex-plugin/plugin.json`, optional `.app.json`, shared `.mcp.json`, and skills-first bundle output
- [gemini-extension-package](./gemini-extension-package): Gemini CLI extension repo with `src/plugin.yaml`, generated `gemini-extension.json`, shared MCP, and packaging-only validation coverage
- [cursor-basic](./cursor-basic): Cursor workspace-config repo with `src/plugin.yaml`, generated `.cursor/mcp.json`, mirrored `.cursor/rules/**`, and no Cursor-native root `AGENTS.md` surface
- [opencode-basic](./opencode-basic): OpenCode workspace-config repo with `src/plugin.yaml`, generated `opencode.json`, shared MCP, and mirrored portable skills
...
Canonical authoring uses `src/plugin.yaml`, `src/mcp/servers.yaml`, and `src/targets/<platform>/...`; committed native Claude/Codex/Gemini/Cursor/OpenCode files in the plugin root are generated managed artifacts.
