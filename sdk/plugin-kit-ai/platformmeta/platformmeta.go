	...
	return []PlatformProfile{
		...
		},
		{
			...
			Contract: TargetContractMeta{
				...
				ProductionClass:        "packaging-only target",
				RuntimeContract:        "workspace-config lane with first-class npm plugin refs, MCP, skills, commands, agents, themes, stable official-style local JS/TS plugins and plugin-local dependencies, JSON/JSONC native import, explicit opt-in user-scope import, config passthrough, and beta custom tools through plugin code",
				...
				Summary:                "OpenCode compiles as a workspace-config lane with canonical repo-local authored outputs for npm plugin refs, shared MCP, skills, commands, agents, themes, stable official-style local JS/TS plugins plus plugin-local package metadata, JSON/JSONC import compatibility for project and explicit user scope, beta custom tools through plugin code, and passthrough support for broader config-only surfaces.",
			},
			SDK: SDKMeta{
