...
const cloudflareCatalogLiveEnvVar = "PLUGIN_KIT_AI_RUN_CLOUDFLARE_LIVE"
const cloudflareCatalogDirEnvVar = "PLUGIN_KIT_AI_E2E_CLOUDFLARE_DIR"
const cloudflareCatalogServerName = "cloudflare-api"
const cloudflareCatalogServerURL = "https://mcp.cloudflare.com/mcp"

func TestCloudflareCatalogLiveAcrossInstalledAgents(t *testing.T) {
	if strings.TrimSpace(os.Getenv(cloudflareCatalogLiveEnvVar)) != "1" {
		t.Skipf("set %s=1 to run real cloudflare catalog live smoke", cloudflareCatalogLiveEnvVar)
	}

	pluginDir := resolveCloudflareCatalogPluginDir(t)
	pluginKitAIBin := buildPluginKitAI(t)
	assertCloudflareCatalogRenderedAndValid(t, pluginKitAIBin, pluginDir)

	server := readRenderedSharedMCPServer(t, pluginDir, cloudflareCatalogServerName)
	assertCloudflareRenderedServer(t, server)
	...
	t.Run("Claude_plugin_dir_list_and_mcp_status", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("claude plugins validate cloudflare: %v\n%s", err, validateOut)
		}
		...
		if !strings.Contains(string(listOut), "cloudflare@inline") {
			t.Fatalf("claude plugins list missing inline cloudflare plugin:\n%s", listOut)
		}
		...
		if err != nil {
			if claudeEnvironmentIssue(string(mcpListOut)) {
				t.Skipf("claude environment is not ready for cloudflare live smoke:\n%s", truncateRunes(string(mcpListOut), 4000))
			}
			t.Fatalf("claude mcp list with --plugin-dir: %v\n%s", err, mcpListOut)
		}
		assertClaudeRemoteCatalogStatus(t, string(mcpListOut), "plugin:cloudflare:cloudflare-api:", cloudflareCatalogServerURL)
	})

	t.Run("Codex_get_and_list_rendered_remote_server", func(t *testing.T) {
		codexBin := installedCodexBinaryOrSkip(t)

		getCmd := exec.Command(codexBin, "mcp", "get", cloudflareCatalogServerName, "--json", "-c", fmt.Sprintf("mcp_servers.%s.url=%q", cloudflareCatalogServerName, cloudflareCatalogServerURL))
		getOut, err := getCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("codex mcp get cloudflare: %v\n%s", err, getOut)
		}
		var getDoc struct {
		...
		if err := json.Unmarshal(getOut, &getDoc); err != nil {
			t.Fatalf("parse codex mcp get cloudflare: %v\n%s", err, getOut)
		}
		if getDoc.Name != cloudflareCatalogServerName || getDoc.Transport.Type != "streamable_http" || strings.TrimSpace(getDoc.Transport.URL) != cloudflareCatalogServerURL {
			t.Fatalf("unexpected codex mcp get cloudflare output:\n%s", getOut)
		}

		listCmd := exec.Command(codexBin, "mcp", "list", "--json", "-c", fmt.Sprintf("mcp_servers.%s.url=%q", cloudflareCatalogServerName, cloudflareCatalogServerURL))
		listOut, err := listCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("codex mcp list cloudflare: %v\n%s", err, listOut)
		}
		var listDoc []struct {
			Name       string `json:"name"`
		...
		if err := json.Unmarshal(listOut, &listDoc); err != nil {
			t.Fatalf("parse codex mcp list cloudflare: %v\n%s", err, listOut)
		}
		assertCodexRemoteCatalogEntry(t, listDoc, cloudflareCatalogServerName, cloudflareCatalogServerURL)
	})
	...
	t.Run("Gemini_extension_validate_link_and_list", func(t *testing.T) {
		...
		if !strings.Contains(listOutput, "cloudflare") || !strings.Contains(listOutput, "MCP servers:") || !strings.Contains(listOutput, cloudflareCatalogServerName) {
			t.Fatalf("gemini extensions list missing cloudflare MCP projection:\n%s", listOutput)
		}
	})
	...
	t.Run("Cursor_isolated_config_shows_auth_boundary", func(t *testing.T) {
		...
		if !strings.Contains(string(listOut), cloudflareCatalogServerName) || !strings.Contains(strings.ToLower(string(listOut)), "needs approval") {
			t.Fatalf("cursor mcp list missing cloudflare approval state:\n%s", listOut)
		}

		enableArgs := cursorCLIArgs(cursorBin, "mcp", "enable", cloudflareCatalogServerName)
		...
		if err != nil {
			t.Fatalf("cursor mcp enable cloudflare with isolated config: %v\n%s", err, enableOut)
		}
		...
		listOut, err = listCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("cursor mcp list after enabling cloudflare: %v\n%s", err, listOut)
		}
		assertCursorRemoteCatalogState(t, string(listOut), cloudflareCatalogServerName)
	})
	...
	t.Run("OpenCode_workspace_serve_startup", func(t *testing.T) {
		...
		cmd.Dir = pluginDir
		cmd.Env = append(os.Environ(), "OPENCODE_SERVER_PASSWORD=plugin-kit-ai-cloudflare-live")
		outPath := filepath.Join(t.TempDir(), "opencode-cloudflare.log")
		...
		if err := cmd.Start(); err != nil {
			t.Fatalf("start opencode serve in cloudflare workspace: %v", err)
		}
...
func assertCloudflareCatalogRenderedAndValid(t *testing.T, pluginKitAIBin, pluginDir string) {
	t.Helper()
	root := RepoRoot(t)
...
func assertCloudflareRenderedServer(t *testing.T, server map[string]any) {
	t.Helper()
	if got := anyString(server["type"]); got != "http" {
		t.Fatalf("generated cloudflare .mcp.json type = %q want http:\n%v", got, server)
	}
	if got := anyString(server["url"]); got != cloudflareCatalogServerURL {
		t.Fatalf("generated cloudflare .mcp.json url = %q want %q:\n%v", got, cloudflareCatalogServerURL, server)
	}
	if _, ok := server["headers"]; ok {
		t.Fatalf("generated cloudflare .mcp.json unexpectedly contains headers:\n%v", server)
	}
}
