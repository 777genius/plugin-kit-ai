...
const notionCatalogLiveEnvVar = "PLUGIN_KIT_AI_RUN_NOTION_LIVE"
const notionCatalogDirEnvVar = "PLUGIN_KIT_AI_E2E_NOTION_DIR"
const notionCatalogServerName = "notion"
const notionCatalogServerURL = "https://mcp.notion.com/mcp"

func TestNotionCatalogLiveAcrossInstalledAgents(t *testing.T) {
	if strings.TrimSpace(os.Getenv(notionCatalogLiveEnvVar)) != "1" {
		t.Skipf("set %s=1 to run real notion catalog live smoke", notionCatalogLiveEnvVar)
	}

	pluginDir := resolveNotionCatalogPluginDir(t)
	pluginKitAIBin := buildPluginKitAI(t)
	assertNotionCatalogRenderedAndValid(t, pluginKitAIBin, pluginDir)

	server := readRenderedSharedMCPServer(t, pluginDir, notionCatalogServerName)
	assertNotionRenderedServer(t, server)
	...
	t.Run("Claude_plugin_dir_list_and_mcp_status", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("claude plugins validate notion: %v\n%s", err, validateOut)
		}
		...
		if !strings.Contains(string(listOut), "notion@inline") {
			t.Fatalf("claude plugins list missing inline notion plugin:\n%s", listOut)
		}
		...
		if err != nil {
			if claudeEnvironmentIssue(string(mcpListOut)) {
				t.Skipf("claude environment is not ready for notion live smoke:\n%s", truncateRunes(string(mcpListOut), 4000))
			}
			t.Fatalf("claude mcp list with --plugin-dir: %v\n%s", err, mcpListOut)
		}
		assertClaudeRemoteCatalogStatus(t, string(mcpListOut), "plugin:notion:notion:", notionCatalogServerURL)
	})

	t.Run("Codex_get_and_list_rendered_remote_server", func(t *testing.T) {
		codexBin := installedCodexBinaryOrSkip(t)

		getCmd := exec.Command(codexBin, "mcp", "get", notionCatalogServerName, "--json", "-c", fmt.Sprintf("mcp_servers.%s.url=%q", notionCatalogServerName, notionCatalogServerURL))
		getOut, err := getCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("codex mcp get notion: %v\n%s", err, getOut)
		}
		var getDoc struct {
		...
		if err := json.Unmarshal(getOut, &getDoc); err != nil {
			t.Fatalf("parse codex mcp get notion: %v\n%s", err, getOut)
		}
		if getDoc.Name != notionCatalogServerName || getDoc.Transport.Type != "streamable_http" || strings.TrimSpace(getDoc.Transport.URL) != notionCatalogServerURL {
			t.Fatalf("unexpected codex mcp get notion output:\n%s", getOut)
		}

		listCmd := exec.Command(codexBin, "mcp", "list", "--json", "-c", fmt.Sprintf("mcp_servers.%s.url=%q", notionCatalogServerName, notionCatalogServerURL))
		listOut, err := listCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("codex mcp list notion: %v\n%s", err, listOut)
		}
		var listDoc []struct {
			Name       string `json:"name"`
		...
		if err := json.Unmarshal(listOut, &listDoc); err != nil {
			t.Fatalf("parse codex mcp list notion: %v\n%s", err, listOut)
		}
		assertCodexRemoteCatalogEntry(t, listDoc, notionCatalogServerName, notionCatalogServerURL)
	})
	...
	t.Run("Gemini_extension_validate_link_and_list", func(t *testing.T) {
		...
		if !strings.Contains(listOutput, "notion") || !strings.Contains(listOutput, "MCP servers:") {
			t.Fatalf("gemini extensions list missing notion MCP projection:\n%s", listOutput)
		}
	})
	...
	t.Run("Cursor_isolated_config_shows_auth_boundary", func(t *testing.T) {
		...
		if !strings.Contains(string(listOut), notionCatalogServerName) || !strings.Contains(strings.ToLower(string(listOut)), "needs approval") {
			t.Fatalf("cursor mcp list missing notion approval state:\n%s", listOut)
		}

		enableArgs := cursorCLIArgs(cursorBin, "mcp", "enable", notionCatalogServerName)
		...
		if err != nil {
			t.Fatalf("cursor mcp enable notion with isolated config: %v\n%s", err, enableOut)
		}
		...
		listOut, err = listCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("cursor mcp list after enabling notion: %v\n%s", err, listOut)
		}
		assertCursorRemoteCatalogState(t, string(listOut), notionCatalogServerName)
	})
	...
	t.Run("OpenCode_workspace_serve_startup", func(t *testing.T) {
		...
		cmd.Dir = pluginDir
		cmd.Env = append(os.Environ(), "OPENCODE_SERVER_PASSWORD=plugin-kit-ai-notion-live")
		outPath := filepath.Join(t.TempDir(), "opencode-notion.log")
		...
		if err := cmd.Start(); err != nil {
			t.Fatalf("start opencode serve in notion workspace: %v", err)
		}
...
func assertNotionCatalogRenderedAndValid(t *testing.T, pluginKitAIBin, pluginDir string) {
	t.Helper()
	root := RepoRoot(t)
...
func assertNotionRenderedServer(t *testing.T, server map[string]any) {
	t.Helper()
	if got := anyString(server["type"]); got != "http" {
		t.Fatalf("generated notion .mcp.json type = %q want http:\n%v", got, server)
	}
	if got := anyString(server["url"]); got != notionCatalogServerURL {
		t.Fatalf("generated notion .mcp.json url = %q want %s:\n%v", got, notionCatalogServerURL, server)
	}
	if _, ok := server["headers"]; ok {
		t.Fatalf("generated notion .mcp.json should not embed headers:\n%v", server)
	}
}
