...
- **Gemini / extension lifecycle:** opt-in real CLI smoke — **`PLUGIN_KIT_AI_GEMINI_BIN=/path/to/gemini`** or **`GEMINI_BIN=/path/to/gemini`**. При необходимости можно явно выключить через **`PLUGIN_KIT_AI_SKIP_GEMINI_CLI=1`**. Тест работает в `Temp HOME` и проверяет `gemini extensions link|config|disable|enable` против rendered reference extension, не трогая реальный user state.
