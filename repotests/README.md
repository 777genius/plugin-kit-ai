...
- **OpenCode / real model smoke:** opt-in real CLI smoke — **`PLUGIN_KIT_AI_RUN_OPENCODE_CLI=1`**, флаг **`-args -opencode-models=...`** для матрицы дешёвых GPT-моделей из OpenCode и **`-args -opencode-mcp-model=...`** для отдельного MCP smoke. Тесты гоняют реальный `opencode run` в generated workspace, проверяют загрузку локального plugin через marker и отдельный live MCP tool call через локальный stdio smoke server. Provider auth/quota/model-availability issues идут в explicit skip c captured JSON evidence, а не в false red.
