...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, string(supportDoc), "real `codex exec` with explicit `-c notify=...` override still reaches the repository-owned notify hook harness, real `codex mcp get --json` and `codex mcp list --json` with explicit `-c mcp_servers...` overrides still expose the projected MCP server contract")
