...
func TestSentryCatalogLiveAcrossInstalledAgents(t *testing.T) {
	if strings.TrimSpace(os.Getenv(sentryCatalogLiveEnvVar)) != "1" {
		t.Skipf("set %s=1 to run real sentry catalog live smoke", sentryCatalogLiveEnvVar)
	}
	...
	t.Run("Claude_plugin_dir_list_and_mcp_status", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("claude plugins validate sentry: %v\n%s", err, validateOut)
		}
		...
		if !strings.Contains(string(listOut), "sentry@inline") {
			t.Fatalf("claude plugins list missing inline sentry plugin:\n%s", listOut)
		}
		...
		if err != nil {
			if claudeEnvironmentIssue(string(mcpListOut)) {
				t.Skipf("claude environment is not ready for sentry live smoke:\n%s", truncateRunes(string(mcpListOut), 4000))
			}
	...
	t.Run("Gemini_extension_validate_link_and_list", func(t *testing.T) {
		...
		if !strings.Contains(listOutput, "sentry") || !strings.Contains(listOutput, "MCP servers:") {
			t.Fatalf("gemini extensions list missing sentry MCP projection:\n%s", listOutput)
		}
	})
	...
	t.Run("Cursor_isolated_config_shows_auth_boundary", func(t *testing.T) {
		...
		if !strings.Contains(string(listOut), "sentry") || !strings.Contains(strings.ToLower(string(listOut)), "needs approval") {
			t.Fatalf("cursor mcp list missing sentry approval state:\n%s", listOut)
		}

		enableArgs := cursorCLIArgs(cursorBin, "mcp", "enable", "sentry")
		...
		if err != nil {
			t.Fatalf("cursor mcp enable sentry with isolated config: %v\n%s", err, enableOut)
		}
		...
		listOut, err = listCmd.CombinedOutput()
		if err != nil {
			t.Fatalf("cursor mcp list after enabling sentry: %v\n%s", err, listOut)
		}
		assertSentryCursorCatalogState(t, string(listOut))
	})
	...
	t.Run("OpenCode_workspace_serve_startup", func(t *testing.T) {
		...
		cmd.Dir = pluginDir
		cmd.Env = append(os.Environ(), "OPENCODE_SERVER_PASSWORD=plugin-kit-ai-sentry-live")
		outPath := filepath.Join(t.TempDir(), "opencode-sentry.log")
		...
		if err := cmd.Start(); err != nil {
			t.Fatalf("start opencode serve in sentry workspace: %v", err)
		}
...
func assertSentryCatalogRenderedAndValid(t *testing.T, pluginKitAIBin, pluginDir string) {
	t.Helper()
	root := RepoRoot(t)
...
func assertSentryCursorCatalogState(t *testing.T, output string) {
	t.Helper()
	lower := strings.ToLower(output)
	if !strings.Contains(output, "sentry") {
		t.Fatalf("cursor mcp list missing sentry entry:\n%s", output)
	}
	if strings.Contains(lower, "auth") || strings.Contains(lower, "ready") {
		return
	}
	if strings.Contains(lower, "connection failed") {
		return
	}
	t.Fatalf("cursor mcp list missing acceptable sentry hosted boundary state:\n%s", output)
}
